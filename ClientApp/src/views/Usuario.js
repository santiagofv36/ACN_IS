    ...
    const Usuario = () => {
        ...
        const validarTelefono = (telefono) => {
            const telefonoRegex = /^(0412|0414|0416|0424|0426)\d{7}$/;
        ...
        const validarFormulario = () => {
            ...
            if (camposVacios.length > 0) {
                Swal.fire('Campos vacíos', `Por favor, complete los siguientes campos: ${camposVacios.join(', ')}.`, 'warning');
                return false;
            }


            if (!/^[a-zA-Z\s]+$/.test(usuario.nombre)) {
                Swal.fire('Nombre inválido', 'El campo del nombre solo debe contener letras y espacios en blanco.', 'warning');
                return false;
            }

            if (!emailRegex.test(usuario.correo)) {
                Swal.fire('Correo inválido', 'Por favor, ingrese un correo electrónico válido.', 'warning');
                return false;
            }

            if (!validarTelefono(usuario.telefono)) {
                Swal.fire('Teléfono inválido', 'Por favor, ingrese un número de teléfono celular válido para Venezuela.', 'warning');
                return false;
            }

            if (usuario.clave.trim() === '') {
                Swal.fire('Contraseña vacía', 'Por favor, ingrese una contraseña.', 'warning');
                camposVacios.push('Contraseña');
                return false;
            }

            if (usuario.idRol === 0) {
                Swal.fire('Rol no seleccionado', 'Por favor, seleccione un rol.', 'warning');
